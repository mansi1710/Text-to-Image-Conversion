...
class AttnGAN():
    ...
    def d_train_step(self, real_256, caption):
        with tf.GradientTape() as d_64_tape, tf.GradientTape() as d_128_tape, tf.GradientTape() as d_256_tape :
            ...
            fake_64, fake_128 = fake_imgs

            uncond_real_logits, cond_real_logits = self.discriminator([real_64, real_128, sent_emb], training=True)
            _, cond_wrong_logits = self.discriminator([real_64[:(self.batch_size - 1)], real_128[:(self.batch_size - 1)], sent_emb[1:self.batch_size]])
    ...
    def g_train_step(self, caption, class_id):
        with tf.GradientTape() as g_tape:
            ...
            word_feature, sent_code = self.cnn_encoder(fake_128, training=True)
        ...
        return g_loss, g_adv_loss, g_kl_loss, g_embed_loss, fake_128
    ...
    def train(self):
        ...
        for idx in range(self.start_iteration, self.iteration):
            ...
            g_loss, g_adv_loss, g_kl_loss, g_embed_loss, fake_128 = self.g_train_step(caption, class_id)
            g_loss += embed_loss
